...
**Результат:** PR-AUC вырос с 0.0112 у наивного бейзлайна до **0.3478**, в **31 раз**. На отложенном дне, который не видели ни валидация, ни подбор гиперпараметров, — **0.4617**. Практический смысл: при 100 алертах в день **84% из них оказываются настоящим отмыванием**.

**Что здесь можно посмотреть помимо метрик:**

- **error analysis по типам схем** обнаружил систематический провал: звёздчатые схемы (fan-in, fan-out, gather-scatter) детектируются с recall 0.53–0.62, цепочечные (cycle, stack, bipartite) — 0.25–0.32; разделение по качеству **в точности совпадает** с разделением по топологии, и это следствие того, что все признаки ограничены одним хопом;
- **измерена цена артефакта данных**: у каналов Wire и Reinvestment ровно ноль позитивов на 653 тысячи транзакций — симулятор просто не генерирует через них схемы, поэтому все метрики считаются в двух режимах, с этим признаком и без; честная оценка переносимости — **0.2282, а не 0.3478**;
...
- **ablation разошёлся с важностями признаков**: gain ставит профиль отправителя почти втрое выше профиля получателя, а удаление групп даёт обратное (−0.049 против −0.098) — измерена цена того, чтобы доверять `feature_importance`;
- показано, что **стандартная оптимизация порога по ожидаемым издержкам вырождается** на распределении с тяжёлым хвостом: формальный оптимум требует 20 тысяч алертов в день;
...
Это оказалось не компромиссом ради памяти, а **одним из двух сильнейших семейств**: удаление всех признаков этой группы стоит −0.091 PR-AUC, больше, чем удаление любой другой поведенческой группы кроме профиля получателя. Содержательно понятно почему — взаимодействие со свежими счетами есть прямая подпись схем с подставными лицами.
...
Разложение прироста: ручные признаки дали ×3.2, смена класса модели ×6.8, подбор гиперпараметров ×1.4. Второй множитель больше первого, но это не значит, что FE второстепенен: колонка «без format» показывает ×134 на одних только вручную построенных признаках — бустингу нечего было бы комбинировать без них.

**XGBoost втрое быстрее RandomForest при втрое лучшем качестве**, и это заниженная оценка разрыва: лес обучался на прореженной выборке в 300 тыс. строк, бустинг на GPU — на полных 3.7 млн, то есть на объёме в 12 раз большем. В пересчёте на строку разница примерно сорокакратная. Прореживание при этом не роскошь, а необходимость: на двух vCPU бесплатного Colab полная выборка для леса недоступна по времени.
...
Порог — топ-2552 скора, что равно числу позитивов в валидационном пуле. Общий recall 0.313.
...
Пунктир — общий recall 0.313. Разрыв примерно вдвое, и граница проходит **точно по топологии**, а не случайно: четыре схемы над пунктиром звёздчатые, три под ним цепочечные, а между ними RANDOM с промежуточным значением, что для схемы со случайной топологией и ожидаемо.
...
**Шаг 4. Что подтверждает измерение.** Recall делится примерно надвое, и граница совпадает с границей по топологии: 0.53–0.62 против 0.25–0.32. Дополнительно `UNMATCHED` (транзакции прикрытия вокруг ядер схем, у которых по определению нет локальной аномалии) даёт 0.075 — худший результат из всех групп, что согласуется с той же логикой.
...
Gain-важности показывают, что модель использует, но не отвечают на вопрос, что она **потеряет** без группы: скоррелированные признаки делят кредит между собой. Прямая проверка — переобучение без каждого семейства (два фолда, базовое значение 0.3552, сравнивать нужно дельты).
...
**Два ранжирования расходятся, и расходятся сильно.** По gain признаки отправителя (3563) почти втрое обходят признаки получателя (1287); по ablation всё наоборот — удаление стороны получателя стоит вдвое дороже (−0.098 против −0.049). Причина в том, что gain распределяется между коррелирующими признаками пропорционально тому, как часто дерево выбрало именно их для разбиения, а не тому, сколько информации в них содержится. Сторона отправителя чаще оказывается «первой под рукой» и набирает gain, но её сигнал дублируется другими группами; сторона получателя набирает меньше, зато незаменима.

Вывод согласуется с двумя независимыми измерениями: в SHAP-разборе конкретного алерта верхние позиции занимают `ri_dt`, `ri_new_3d`, `ri_new_7d`, а по типам схем FAN-IN (0.589) и GATHER-SCATTER (0.568) детектируются лучше, чем FAN-OUT (0.531). Схемы, где аномален **получатель**, ловятся надёжнее.
...
**Транзитность.** Гипотеза «на счёте-прокладке втекает ≈ вытекает» выглядела как самая содержательная часть feature engineering. Удаление всех шести признаков стоит **−0.0030 PR-AUC** при разбросе по фолдам ±0.074, то есть неотличимо от нуля. Вероятная причина: отношение объёмов фиксирует транзит только когда окно точно совпадает со схемой, тогда как счётчики новых контрагентов ловят ту же схему устойчивее и вытесняют транзитность из разбиений.

**Ансамблирование.** Оптимальный вес рангового бленда оказался равен **1.0**: любая примесь MLP строго вредит, и качество падает монотонно — 0.3478, 0.3372, 0.3304, 0.3242, 0.3107 при весах XGBoost от 1.0 до 0.5. Мета-модель (0.3266) тоже не обошла одиночный бустинг. MLP обучен на тех же признаках, слабее в 1.7 раза и ошибается там же, где бустинг. Стекинг помогает, когда модели ошибаются **по-разному** — здесь это условие не выполняется.
...
**Pooled и per-fold PR-AUC несопоставимы.** При сравнении стекинга с одиночной моделью легко напечатать рядом два числа, посчитанных по-разному: одно на объединённом пуле всех дней, другое как среднее по фолдам. Pooled смешивает дни с разной базовой частотой позитивов и на этих данных даёт величину примерно на 0.07 ниже per-fold для одной и той же модели — достаточно, чтобы «доказать» несуществующий прирост от блендинга. В финальной версии все конфигурации, включая чистый XGBoost с весом 1.0, прогоняются через один и тот же цикл по фолдам.

## Ограничения

- **Данные синтетические.** AMLSim генерирует схемы по правилам, и часть сигнала — артефакты генератора. Самый явный: у Wire и Reinvestment ровно ноль позитивов на 653 тысячи транзакций, у кросс-валютных переводов тоже ноль. Модель выучивает `fmt_Wire` как детерминированное отрицательное правило, и удаление группы `fmt` стоит −0.128 PR-AUC — больше, чем удаление любой поведенческой группы. Честная оценка переносимости — **0.2282** (колонка «без format»).
...
- **Экономия считается от нуля.** «63% экономии» в бизнес-блоке измерена относительно сценария «не проверяем вообще ничего», а не относительно существующего rule-based процесса банка. Это нижняя граница эффекта.
